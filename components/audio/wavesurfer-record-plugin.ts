...
class RecordPlugin extends BasePlugin<RecordPluginEvents, RecordPluginOptions> {
  ...
  private originalOptions:
    | { cursorWidth: number; interact: boolean }
    | undefined
  private timer: Timer
  ...
  constructor(options: RecordPluginOptions) {
    super({
      ...
      scrollingWaveformWindow:
        options.scrollingWaveformWindow ?? DEFAULT_SCROLLING_WAVEFORM_WINDOW,
  ...
  public renderMicStream(stream: MediaStream): MicStream {
    ...
    const windowSize = Math.floor(
      (this.options.scrollingWaveformWindow || 0) * audioContext.sampleRate
    )
    ...
    const drawWaveform = () => {
      ...
      if (this.options.scrollingWaveform) {
        const newLength = Math.min(
          windowSize,
          this.dataWindow ? this.dataWindow.length + bufferLength : bufferLength
        )
        ...
        if (this.dataWindow) {
          ...
          tempArray.set(
            this.dataWindow.slice(-newLength + bufferLength),
            startIdx
          )
        }
  ...
  public async startMic(
    options?: RecordPluginDeviceOptions
  ): Promise<MediaStream> {
  ...
  public async startRecording(options?: RecordPluginDeviceOptions) {
    ...
    const mediaRecorder =
      this.mediaRecorder ||
      new MediaRecorderPolyfill(stream, {
